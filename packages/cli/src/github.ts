...
export class GitHubClient {
  constructor(
    ...
    private readonly retry: { maxAttempts?: number; sleep?: (milliseconds: number) => Promise<void> } = {},
  ) {}
  ...
  async getIssueContext(repo: RepoSlug, issueNumber: number): Promise<Issue> {
    const [issue, comments] = await Promise.all([
      this.request<GitHubIssue>(`/repos/${repo}/issues/${issueNumber}`, { method: "GET" }),
      this.request<GitHubIssueComment[]>(`/repos/${repo}/issues/${issueNumber}/comments?per_page=100`, { method: "GET" }),
    ]);
    ...
    return {
      ...
      attachments: extractUrls([issue.body ?? "", ...normalizedComments.map((comment) => comment.body)]),
    };
  }
  ...
  private async request<T>(path: string, init: RequestInit): Promise<T> {
    ...
    const sleep = this.retry.sleep ?? ((milliseconds: number) => new Promise<void>((resolve) => setTimeout(resolve, milliseconds)));
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        if (isGithubResponseError(error) || attempt === maxAttempts) throw error;
