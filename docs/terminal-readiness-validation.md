...
The Click, pflag, jq, and picocli additions were run as fix-and-retest exercises. The first attempts exposed concurrent global-config corruption, Bun-only init defaults, a 120-second cold-start limit, unavailable legacy image tags, missing native/Java detection, Gradle wrapper network dependence, ambiguous sandbox paths, sandbox-root-relative uploads, and file assertions evaluated outside the repository workdir. Each harness defect received a regression test and implementation fix. Fresh pflag, jq, and picocli runs then completed from the same one-line prompts with verified evidence, no comments, and cleanup. No reproduction command or issue-specific hint was supplied to Cursor.
...
- The regression manifest contains 28 real issues across Rust, TypeScript, Python, Go, C, and Java with fixed expected dispositions.
