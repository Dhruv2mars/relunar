...
The Click, pflag, and jq additions were run as fix-and-retest exercises. The first attempts exposed concurrent global-config corruption, Bun-only init defaults, a 120-second cold-start limit, unavailable legacy image tags, missing native-project detection, ambiguous sandbox paths, sandbox-root-relative uploads, and file assertions evaluated outside the repository workdir. Each harness defect received a regression test and implementation fix. Fresh pflag and jq runs then completed from the same one-line prompts with verified control evidence, no comments, and cleanup. No reproduction command or issue-specific hint was supplied to Cursor.
...
- The regression manifest contains 27 real issues across Rust, TypeScript, Python, Go, and C with fixed expected dispositions.
